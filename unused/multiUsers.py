# multiUsers.py
#from https://github.com/raymondino/InsiderThreat-StreamReasoningUseCase/blob/master/mychoco/multiUsers.py
...
def multiUserAnnotate(userList, dailyStartDic, dailyEndDic, usbDriveUsageFrequency):
	...
	def logon(record,outfile):
		...
		print('%s%s-event %shasAction %slogon_%s . %s' %(ex,record[3],ex,ex,id,tsToStr(timestamp)),file=outfile)
